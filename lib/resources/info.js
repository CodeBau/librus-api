...
function parseBehaviorSubject($, row) {
  ...
  detailTable.find("tbody > tr").each((_, detailRow) => {
    ...
    const value = normalizeText($(cells[2]).text());
    if (!value) return;
...
module.exports = class Info extends Resource {
  ...
  getAccountInfo() {
    let parser = ($, element) => {
      ...
      $("#body table tbody tr").each((_, tr) => {
        ...
        if (/Nr\s+w\s+dzienniku/i.test(thText)) {
      ...
      if (studentIndex === null) {
        const rawIndex = $("#body > div > div > table > tbody > tr:nth-child(3) > td")
          .text()
          .trim();
  ...
  /**
   * Get student number in journal ("Nr w dzienniku")
  ...
  getStudentNumber() {
    return this.getAccountInfo().then((info) => {
      return info && info.student ? info.student.index : null;
    });
  }
  ...
  getGrades() {
    let parser = ($, row) => {
      ...
      if (name)
        return {
          name,
          semester: [semester(2), emptySemester(), semester(5), emptySemester()],
